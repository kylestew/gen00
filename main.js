...
import createAnimations from "./lib/animations";
...
const activate = async () => {
  ...
  const musicPhrase = () => {
    ...
    phrase.slice(0, sliceLength).forEach((pc, i) => {
      ...
      Tone.Transport.scheduleOnce((time) => {
        ...
        trumpet.triggerAttackRelease(`${pc}${trumpetOct}`, duration);
      ...
      Tone.Transport.scheduleOnce((time) => {
        ...
        trombone.triggerAttackRelease(`${pc}${trumpetOct}`, duration);
  ...
  Tone.loaded().then(() => {
    ...
    trombone.connect(tromboneReverb);
